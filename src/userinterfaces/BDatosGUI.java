...
public class BDatosGUI extends JDialog {

    public BDatosGUI(java.awt.Frame parent, int id) {
        super(parent, true);
        this.Id = id;
        initComponents();
    ...
    private void initComponents() {
        ...
        jLabel13.setText("Progreso de borrado »");
        ...
        jLabel3.setText(" Mediciones Insertadas");
    ...
    private void ok(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ok
        ...
        try {
            if (this.jComboBox1.getSelectedIndex() != 0 &&
                    ...
                    this.dtm1.getRowCount() != 0) {
                int i, n;

                asunto = A.getId(this.jComboBox1.getSelectedItem().toString());
                site = S.getId(this.jComboBox2.getSelectedItem().toString());

                n = this.dtm1.getRowCount();

                for (i = 0; i < n; i++) {
                    if ((Boolean) dtm1.getValueAt(i, 1)) // Borrar
                    {
                        medidas_borradas.add(i);//                 root = SwingUtilities.getRoot(this);
//                 
...
//                        this.jLabel13.setText("Borrando medida "+fichero+" ...");
//                        update(this.getGraphics());
//                        
...
//                }
                ...
                JOptionPane.showMessageDialog(this, "No hay nada que borrar", "ADVERTENCIA", JOptionPane.WARNING_MESSAGE);
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "Fallo en el proceso de borrado ", "ERROR", JOptionPane.ERROR_MESSAGE);
            root.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
        } catch (Exception e) {
    ...
    private void Limpiar() {
        ...
        //Por qué 13 a fuego? Filas añadidas conforme hay que mostrarlas en el jTable
    /*
        for (int i=0;i<13;i++){
         ...
         */
        //medidas_insertadas = new ArrayList<String>();
        medidas_borradas = new ArrayList<Integer>();
    }
...
private void CambioSite(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CambioSite
    ...
    if ((this.jComboBox2.getSelectedIndex() != 0) && (this.jComboBox2.getSelectedItem() != null) && (this.jComboBox2.getSelectedItem() != "")) {
        try {
             ...
             */
            //Generalización del método quedándonos con los 3 primeros caracteres del nombre
            if (nombre_site.length() > 3) {
                nombre_site = nombre_site.substring(0, 3);
            }
            tabla_datos = "Datos" + nombre_site;
            tabla_descripcion = "Descripcion" + nombre_site;
            ...
            // Relleno las medidas insertadas
            ArrayList<String[]> insercion = I.getInsercion(asunto, site);
            int n = insercion.size();

            for (int i = 0; i < n; i++) {
                //Añadimos filas conforme las vamos necesitando
                Object[] fila = new Object[2];
                fila[0] = null;
                fila[1] = new Boolean(false);
                dtm1.addRow(fila);

                this.jTable1.setValueAt(insercion.get(i)[0], i, 0);
            }
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Fallo al consultar la base de datos", "ERROR", JOptionPane.ERROR_MESSAGE);
    ...
    private Insercion I = new Insercion();    // Tablas
    private DefaultTableModel dtm1;    // Variables
    private int Id;
    private int asunto;
    private int site;
    //private ArrayList<String> medidas_insertadas = new ArrayList<String>();     //Ruta
    private ArrayList<Integer> medidas_borradas = new ArrayList<Integer>();     //Fila
