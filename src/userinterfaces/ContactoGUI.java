...
public class ContactoGUI extends JDialog {
     
    public ContactoGUI(java.awt.Frame parent) {
       super(parent, true);
       initComponents(); 
    ...
    private void initComponents() {
        ...
        setBounds((screenSize.width-422)/2, (screenSize.height-224)/2, 422, 224);
...
if ((this.jComboBox1.getSelectedIndex()!=0)||(this.jTextField5.getText().length()!=0) || (this.jTextField6.getText().length()!=0)){
    try{
        Cliente C=new Cliente();
        int cliente=C.getId(this.jComboBox1.getSelectedItem().toString());
        String nombre=this.jTextField5.getText();
        String mail=this.jTextField6.getText();
        String telefono=this.jTextField7.getText();
        if (!C.NuevoContacto(nombre, cliente, mail, telefono,false)){
            JOptionPane.showMessageDialog(this, "Contacto introducido correctamente","INFORMACIÓN",JOptionPane.INFORMATION_MESSAGE);
            dispose();
        } else{
            JOptionPane.showMessageDialog(this, "Contacto ya existente","ADVERTENCIA",JOptionPane.WARNING_MESSAGE);
        }        
    } catch (SQLException e){
        e.printStackTrace();
        JOptionPane.showMessageDialog(this, "Fallo al introducir el contacto","ERROR",JOptionPane.ERROR_MESSAGE);
    }   
} else{
...
private void ValidarMail(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_ValidarMail
